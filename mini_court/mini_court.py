...
class MiniCourt:
    ...
    def draw_background_rectangle(self, frame):
        ...
        out = frame.copy()
        alpha = 0.5
        mask = shapes.astype(bool) # convert to boolean mask
        out[mask] = cv2.addWeighted(frame,alpha,shapes,1-alpha,0)[mask] # blend the shapes with the frame
        return out
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self,
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ball_boxe = ball_boxes[frame_num][1] #get ball bbox from list of bboxes. ball_boxes=[ frame 1 {id: [x1,y1,x2,y2]}, frame 2 {id: [x1,y1,x2,y2]}..frame n {id: [x1,y1,x2,y2]}]
            # ball_boxe = [x1,y1,x2,y2] 
            ball_position = get_center_of_bbox(ball_boxe)
            closest_player_id_to_ball = min(player_bbox.keys(), key=lambda k: measure_distance(ball_position, get_center_of_bbox(player_bbox[k])))
            ...
            for player_id, bbox in player_bbox.items():
                ...
                # Get the closest keypoint in pixel 
